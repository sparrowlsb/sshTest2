...
public interface FundTransactionDao  extends PagingAndSortingRepository<FundTransactionPo, Integer> {
    ...
    @Query(value = "SELECT id,user_id,type,status,trader_money,fund_id,management_cost,transaction_date,fund_count,fund_price,management_fee  from FUND_TRANSACTION where id=:fundOrderId and type=:type and status =:status ", nativeQuery = true)
    ...
    @Query(value = "SELECT id,user_id,type,status,trader_money,fund_id,management_cost,transaction_date,fund_count,fund_price,management_fee  from FUND_TRANSACTION where user_id=:userId and type='BUY' and status =0  order by transaction_date desc", nativeQuery = true)
